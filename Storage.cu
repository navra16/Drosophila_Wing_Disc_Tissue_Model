...
void Storage::print_VTK_File(void) {
    ...
    std::string initial = "Animation_realistic/04_25_26/_12layer_strain_test_plus_linear_volume_interpolation_";
...
// Count valid edges first to get accurate cell count
    ...
    for (int edge = 0; edge < SYSTEM->coordInfoVecs.num_edges; edge++) {
        if (SYSTEM->coordInfoVecs.edges2Nodes_1[edge] != INT_MAX &&
            ...
            SYSTEM->coordInfoVecs.edges2Nodes_2[edge] != -INT_MAX &&
            SYSTEM->coordInfoVecs.edges2Nodes_1[edge] >= 0 &&
            SYSTEM->coordInfoVecs.edges2Nodes_2[edge] >= 0) {
            validEdgeCount++;
        }
    }
    
    int numEdges = validEdgeCount;
    int numCells = numEdges + 1;  // +1 for LJ Particle
    int numNumsInCells = (3 * numEdges) + 2;  // 3 per edge (count + 2 nodes) + 2 for LJ particle
    
    ofs << "CELLS " << numCells << " " << numNumsInCells << std::endl;
    
    // Write edges as cells
    for (int edge = 0; edge < SYSTEM->coordInfoVecs.num_edges; edge++) {
        if (SYSTEM->coordInfoVecs.edges2Nodes_1[edge] != INT_MAX &&
            SYSTEM->coordInfoVecs.edges2Nodes_2[edge] != INT_MAX &&
            SYSTEM->coordInfoVecs.edges2Nodes_1[edge] != -INT_MAX &&
    ...
    for (int i = 0; i < numEdges; i++) {  // Use numEdges (valid count), not num_edges
        ofs << 3 << std::endl;  // Type 3 = line
    }
    ofs << 1 << std::endl;  // Type 1 = vertex (for LJ particle)
		
	
		ofs << "CELL_DATA " << numCells << std::endl;
    ofs << "SCALARS Strain double 1" << std::endl;  // Note: add "1" for number of components
    ...
    ofs << std::fixed << 0.1 << std::endl;  // For LJ particle
